...
export async function getSettings(userId) {
  ...
  if (rows.length === 0) {
    ...
    await sql`INSERT INTO settings (user_id, budget, github_org) VALUES (${userId}, 15000, '')`;
    return { budget: 15000, githubOrg: '' };
  }
...
export async function saveSettings(userId, data) {
  await sql`
    ...
    VALUES (${userId}, ${data.budget || 15000}, ${data.githubOrg || ''})
    ON CONFLICT (user_id) DO UPDATE SET
      budget = ${data.budget || 15000}, github_org = ${data.githubOrg || ''}
  `;
}
