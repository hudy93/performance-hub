...
// Tagged template that mimics @vercel/postgres sql`` interface
...
function rowToEmployee(row, personalGoals, teamGoals, extras, assessments) {
  return {
    ...
    lastReview: row.last_review ? (row.last_review instanceof Date ? row.last_review.toISOString().split('T')[0] : row.last_review) : null,
...
export async function createEmployee(userId, data) {
  const { rows } = await sql`
    INSERT INTO employees (user_id, name, role, department, avatar, current_salary,
      salary_band_min, salary_band_mid, salary_band_max, market_rate,
    ...
    VALUES (${userId}, ${data.name}, ${data.role}, ${data.department}, ${data.avatar},
      ...
      ${data.salaryBand?.max || 0}, ${data.marketRate || 0},
...
export async function updateEmployee(userId, employeeId, data) {
  ...
  await sql`
    UPDATE employees SET
      ...
      performance_score = ${data.performanceScore || 3.0},
...
export async function getSettings(userId) {
  ...
  if (rows.length === 0) {
    ...
    await sql`INSERT INTO settings (user_id, budget, github_org, inflation) VALUES (${userId}, 15000, '', 3.2)`;
    return { budget: 15000, githubOrg: '', inflation: 3.2 };
  }
  return { budget: rows[0].budget, githubOrg: rows[0].github_org, inflation: parseFloat(rows[0].inflation) || 3.2 };
}

export async function saveSettings(userId, data) {
  await sql`
    INSERT INTO settings (user_id, budget, github_org, inflation)
    VALUES (${userId}, ${data.budget || 15000}, ${data.githubOrg || ''}, ${data.inflation || 3.2})
    ON CONFLICT (user_id) DO UPDATE SET
      budget = ${data.budget || 15000}, github_org = ${data.githubOrg || ''}, inflation = ${data.inflation || 3.2}
  `;
}
